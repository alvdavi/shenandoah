...
void ShenandoahHeap::entry_uncommit(double shrink_before, size_t shrink_until) {
  ...
  ShenandoahConcurrentPhase gc_phase(msg, ShenandoahPhaseTimings::conc_uncommit, "Uncommit", GLOBAL, true /* log_heap_usage */);
  EventMark em("%s", msg);
...
GrowableArray<MemoryPool*> ShenandoahHeap::memory_pools() {
  ...
  if (mode()->is_generational()) {
    ...
    memory_pools.append(_global_memory_pool);
  }
  return memory_pools;
}
